...
class D_StackDilateBlock(nn.Module):
    def __init__(self, in_c, out_c_per, kernel_size, stride, paddings, dilations, device):
        ...
        self.layers = \
            nn.ModuleList([ nn.Conv1d(in_channels=in_c, 
...
def calc_negpad_transpose(lin, lout, stride, k, d, half=False):
    negpad =  (lin-1)*stride -lout + d*(k-1)+1
    if half:
        negpad = math.floor(negpad / 2)
    return negpad
...
class G_StackDilateBlock(nn.Module):
    def __init__(self, in_c, out_c_per, kernel_size, stride, neg_paddings, dilations, device):
        ...
        self.layers = \
            nn.ModuleList([ nn.ConvTranspose1d(in_channels=in_c, 
                        out_channels=out_c_per, 
                        kernel_size=kernel_size, 
                        stride=stride,
...
#         dilation = 1
#         neg_padding = calc_negpad_transpose(lin=2**4, lout=2**6, stride=4, k=24, d=dilation, half=False) 
#         self.neg_paddings.append(neg_padding)
#         self.tconv1 = nn.ConvTranspose1d(in_channels=16*d, 
#                                          out_channels=8*d, 
#                                          kernel_size=24, 
#                                          stride=4, 
#                                          padding=0,
#                                          dilation=dilation)  # (N, 8d, 64)
        dilations = [1]
...
#         dilation = 2
#         neg_padding = calc_negpad_transpose(lin=2**6, lout=2**8, stride=4, k=24, d=dilation, half=False)
#         self.neg_paddings.append(neg_padding)
#         self.tconv2 = nn.ConvTranspose1d(in_channels=8*d, 
#                                          out_channels=4*d, 
#                                          kernel_size=24, 
...
#         dilation = 4
#         neg_padding = calc_negpad_transpose(lin=2**8, lout=2**10, stride=4, k=24, d=dilation, half=False)
#         self.neg_paddings.append(neg_padding)
#         self.tconv3 = nn.ConvTranspose1d(in_channels=4*d, 
#                                          out_channels=2*d, 
#                                          kernel_size=24, 
...
#         dilation = 2
#         neg_padding = calc_negpad_transpose(lin=2**10, lout=2**12, stride=4, k=24, d=dilation, half=False)
#         self.neg_paddings.append(neg_padding)
#         self.tconv4 = nn.ConvTranspose1d(in_channels=2*d, 
#                                          out_channels=d, 
#                                          kernel_size=24, 
...
#         dilation = 1
#         neg_padding = calc_negpad_transpose(lin=2**12, lout=2**14, stride=4, k=24, d=dilation, half=False)
#         self.neg_paddings.append(neg_padding)
#         self.tconv5 = nn.ConvTranspose1d(in_channels=d, 
#                                          out_channels=1, 
#                                          kernel_size=24, 
...
#                                          dilation=dilation)  # (N, 1, 16384)
    ...
    def forward(self,x):
        ...
        out = self.tanh_output(self.last_conv(self.relu5(self.gsdb5(out))))
#         out = self.relu0(self.dense(x).view(-1, 16*self.d, 16))
#         out = self.relu1(self.tconv1(out )[:,:,self.neg_paddings[0]:] )
#         out = self.relu2(self.tconv2(out )[:,:,self.neg_paddings[1]:] )
#         out = self.relu3(self.tconv3(out )[:,:,self.neg_paddings[2]:] )
#         out = self.relu4(self.tconv4(out )[:,:,self.neg_paddings[3]:] )
#         out = self.tanh_output(self.tconv5(out )[:,:,self.neg_paddings[4]:] )
